...
class RenderWithNormalBuffer : CustomPass
{
    ...
    {
        depthPrepassIds = new ShaderTagId[] { HDShaderPassNames.s_DepthForwardOnlyName, HDShaderPassNames.s_DepthOnlyName, HDShaderPassNames.s_TransparentDepthPrepassName};
        forwardIds = new ShaderTagId[] { HDShaderPassNames.s_ForwardOnlyName, HDShaderPassNames.s_ForwardName, HDShaderPassNames.s_SRPDefaultUnlitName};
    }
    ...
    protected override void Execute(CustomPassContext ctx)
    {
        ...
        bool isDepthNormal = injectionPoint == CustomPassInjectionPoint.BeforePostProcess;
        ...
        var result = new RendererListDesc(ids, ctx.cullingResults, ctx.hdCamera.camera)
        {
            ...
            renderQueueRange = GetRenderQueueRange(RenderQueueType.Transparent),
        ...
        if (isDepthNormal)
        {
            ...
            CoreUtils.SetRenderTarget(ctx.cmd, ctx.cameraNormalBuffer, ctx.customDepthBuffer.Value, ClearFlag.None);
            // CoreUtils.SetRenderTarget(ctx.cmd, ctx.cameraNormalBuffer, ClearFlag.Depth);
        ...
        if (isDepthNormal)
        {
            ...
            CoreUtils.SetKeyword(ctx.cmd, "WRITE_NORMAL_BUFFER", ctx.hdCamera.frameSettings.litShaderMode == LitShaderMode.Deferred);
        }
    }
