...
class SlightBlurNormals : CustomPass
{
    [Range(0, 128)]
    ...
    protected override void Setup(ScriptableRenderContext renderContext, CommandBuffer cmd)
    {
        if (compositeShader == null)
            compositeShader = Resources.Load<Shader>("CompositeBlurNormals");
        ...
        downSampleBuffer = RTHandles.Alloc(
            ...
            colorFormat: GraphicsFormat.R16G16B16A16_SNorm, // We don't need alpha in the blur
    ...
    protected override void Execute(CustomPassContext ctx)
    {
        ...
        if (compositeMaterial != null && radius > 0)
        {
            ...
            for (int i = 0; i < Math.Ceiling(Math.Log(radius, 2)); i++)
            {
    ...
    {
        RTHandle source = ctx.cameraNormalBuffer;
