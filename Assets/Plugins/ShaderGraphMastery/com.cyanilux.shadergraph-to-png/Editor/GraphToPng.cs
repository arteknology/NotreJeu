...
public class GraphToPng {
	...
	public static string Save(int width, int height, Color[] pixels, string name) {
		...
		Debug.Log($"Saved graph at {path}");
		AssetDatabase.Refresh();
		return path;
	}

	static string GetUniquePathName(string name) {
		string path = $"{Application.dataPath}{savePath}/{name}.png";
		int i = 0;
		while (System.IO.File.Exists(path)) {
			path = $"{Application.dataPath}{savePath}/{name}{i:000}.png";
			i++;
		}
		return path;
	}
	...
	private static void EditorUpdate() {
		...
		float t = 0.01f;
		if (doProccessingEnumerator.Current is float current)
